    ...
    if (filteredRequests.length === 0) {
        return (
            <div className="py-20 text-center">
                ...
                <h3 className="text-base font-bold text-slate-600">No leave requests</h3>
                <p className="text-sm text-slate-400 mt-1 mb-6">{showEmployeeColumn ? 'Organization requests will appear here.' : 'Your requests will appear here after you apply.'}</p>
                <Button variant="accent" size="sm" className="rounded-xl px-6" onClick={onShowModal}>
                    <Plus className="w-4 h-4 mr-2" />
                    {showEmployeeColumn ? 'Add Leave for Employee' : 'Submit First Request'}
                </Button>
            </div>
        );
    }
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50/30">
                            {showEmployeeColumn && <th className="px-6 py-4 border-b border-slate-100">Employee</th>}
                        ...
                        <div key={req.id} className="p-5 space-y-4">
                            ...
                            {(req.status === 'Pending' || (req.status === 'Approved' && viewMode === 'organization')) && (
                                ...
                                >
                                    ...
                                    {viewMode === 'organization' ? 'Cancel (HR)' : 'Cancel Request'}
                                </Button>
                            )}
